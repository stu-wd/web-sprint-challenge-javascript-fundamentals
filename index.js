...
//nestedFunction lives inside the code block called myFunction. Because the variable was defined in its parent, nestedFunction has access.
...
function lowerCaseNames(array){
  let lowerCaseArray = array.map(item => item["animal_name"].toLowerCase());
...
lowerCaseNames(zooAnimals);
 ...
 function lowPopulationAnimals (array) {
  let lowPopArray = array.filter(item => item["population"] < 5);
  return lowPopArray;
}
...
lowPopulationAnimals(zooAnimals);
 ...
 function USApop (array){
  let pop = array.reduce((acc, current) => acc + current["population"], 0);
  return pop;
}
